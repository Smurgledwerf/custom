...
__all__ = ['CustomPipelineCanvasWdg']
...
class CustomPipelineCanvasWdg(BaseRefreshWdg):
    '''Pipeline Widget'''
...
#        canvas.add_behavior( { # MTM This was the zoom .. also see the behavior attached to paint
#        "type": 'wheel',
#        "cbjs_action": '''
...
#        paint.add_behavior( { # MTM This was the zoom .. also see the hebavior attached to canvas
#        "type": 'wheel',
#        "cbjs_action": '''
#            spt.pipeline.init(bvr);
#            var scale = spt.pipeline.get_scale();
...
#        } )
    ...
    def get_node(my, name):
        ...
        //spt.pipeline.ban_chars(node, value); //MTM inserted this
...
spt.pipeline.get_mouse_position = function(mouse_411) {
    ...
    var rel_pos = {x: mouse_411.curr_x - canvas_pos.x, y: mouse_411.curr_y - canvas_pos.y}
    return rel_pos;
}
...
spt.pipeline.ban_chars = function(node, value) { // MTM Created This
    ...
    bad_char = '';
    boolio = true;
    for(var r = 0; r < bad_chars.length; r++){
        if(value.indexOf(bad_chars[r]) != -1){
            boolio = false;
            bad_char = bad_chars[r];
        }
    }
    if(boolio){
	    node.spt_name = value;
	    ...
	    node.spt_name = 'RENAME';
	    node.setAttribute("spt_element_name", 'RENAME');
	    node.setAttribute("title", 'RENAME');
	    text.innerHTML = 'RENAME';
	    input.value = 'RENAME';
            spt.alert("Character '" + bad_char + "' is not allowed in a pipeline. Please only use characters a-z, A-Z, 0-9, spaces underscores and dashes.");
    }
...
spt.pipeline.rename_node = function(node, value) {
    ...
    text.innerHTML = value;
    input.value = value;
    spt.pipeline.ban_chars(node, value);
...
spt.pipeline.import_nodes = function(group, xml_nodes) {
    ...
    //attach original name and process code to node as attributes
    
    // find the left most and top most position
    var left = null;
    var top = null;
    var server = TacticServerStub.get();
    process_expr = "@SOBJECT(config/process['pipeline_code','" + group + "'])";
    processes = server.eval(process_expr);
    for(var r = 0; r < processes.length; r++){
        for(var v = 0; v < xml_nodes.length; v++){
            if(xml_nodes[v].getAttribute("name") == processes[r].process){
                xml_nodes[v].setAttribute('original_name',processes[r].process); 
                xml_nodes[v].setAttribute('process_code',processes[r].code); 
            }
        }
    }
...
spt.pipeline.kill_bad_chars = function(value) {
    value = value.replace("'",'');
    value = value.replace('"','');
    value = value.replace(':','');
    return value
...
spt.pipeline.export_groups = function() {
 function oc(a){
    var o = {};
    ...
    for(group_name in groups){
        ...
        if (typeof(group_name) == 'undefined') {
            nodes_temp = spt.pipeline.get_all_nodes(group_name);  
            connectors_temp = canvas.connectors;
            alert('IN WEIRD PLACE. canvas.connectors - not a function');
        }
        else {
            nodes_temp = spt.pipeline.get_nodes_by_group(group_name);
            connectors_temp = group.get_connectors();
            dangling_connectors = group.get_dangling_connectors();
        }
        for(var r =0; r < nodes_temp.length; r++){
            nodes.push(nodes_temp[r]);
        }
        for(var r =0; r < connectors_temp.length; r++){
            connectors.push(connectors_temp[r]);
        }
    ...
    //var sortfunc = function(a,b) {  //MTM COMMENT THIS OUT...We are sorting it differently below
    ...
    new_order_nodes = [];
    added = [];
    while(new_order_nodes.length != nodes_array.length){
        smallest_x = 1000000000;
        smallest_r = -1;
        for(var r = 0; r < nodes_array.length; r++){
            if(!(r in oc(added))){
                x_pos = spt.pipeline.get_position(nodes_array[r]).x;
                if(x_pos < smallest_x){
                    smallest_x = x_pos;
                    smallest_r = r;
                } 
        ...
        new_order_nodes.push(nodes_array[smallest_r]);
        added.push(smallest_r);
    }
    //nodes = nodes_array; 
    nodes = new_order_nodes; 
    for(var r = 0; r < connectors.length; r++){
        var from_node_name = connectors[r].get_from_node().spt_name;
        var to_node_name = connectors[r].get_to_node().spt_name;
    }
    ...
    for (var i = 0; i < nodes.length; i++) {
        ...
        for (var key in properties) {
            ...
            if(key == 'description'){
                value = spt.pipeline.kill_bad_chars(value)
                value = value
            }
            if(key != 'names' && key != 'namedItem' && key != 'item' && key != 'iterator' && key != '@@iterator'){
                xml += ' '+key+'="'+value+'"';
            }
        }
        xml += '/>\n';
...
spt.pipeline.export_group = function(group_name) {
    ...
    //alert('in export group - singular');
    var data = spt.pipeline.get_data();
    var canvas = spt.pipeline.get_canvas();
    ...
    if (group == null) {
        var xml = '<?xml version="1.0" encoding="UTF-8"?>\n';
        //alert('leaving export group - singular 1');
        return xml;
    }
    ...
    var sortfunc = function(a,b) {
        var pos_a = spt.pipeline.get_position(a);
        var pos_b = spt.pipeline.get_position(b);
        //alert('leaving export group - singular 2');
        return pos_a.x - pos_b.x;
    }
    nodes_array.sort(sortfunc);
    nodes = nodes_array;
    ...
    //alert('leaving export group - singular 4');
    return xml;
