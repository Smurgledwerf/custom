import tacticenv, time, os
from client.tactic_client_lib import TacticServerStub
#doing = open('what_have_i_done', 'w')
server = TacticServerStub.get(protocol="xmlrpc")
server.start('CALCULATING COSTS')
orders = server.eval("@SOBJECT(twog/order['id','<','278']['@ORDER_BY','code desc'])")
group_dict = server.eval("@SOBJECT(sthpw/login_group)")
groups = {}
for g in group_dict:
    grp = g.get('login_group')
    groups[grp] = g
equipment_dict = server.eval("@SOBJECT(twog/equipment)")
equipment_lookup = {}
...
for order in orders:
    ...
    for title in titles:
        ...
        twhs = server.eval("@SOBJECT(sthpw/work_hour['is_billable','1']['title_code','%s'])" % title_code)
        ...
        for twh in twhs:
            if twh.get('task_code') in [None,'']:
                ...
                wheq = server.eval("@SOBJECT(twog/equipment_used['work_order_code','%s'])" % twh.get('code'))
                for equip in wheq:
                    ...
                    if eq:
                        ...
                        if lengths not in [None,'']:
                            ...
                            if ('E_DELIVERY' in equip.get('name') or 'STORAGE' in equip.get('name')) and units in ['hr','items']:
                                units = 'gb'
                                server.update(equip.get('__search_key__'), {'units': 'gb'})
        ...
        for proj in projs:
            ...
            for wo in wos:
                ...
                if task:
                    ...
                    whs = server.eval("@SOBJECT(sthpw/work_hour['is_billable','1']['task_code','%s'])" % task_code)
                    for wh in whs:
                        wh_code = wh.get('code')
                        # make cost here
                        cost_to_add = 0
                ...
                server.update(wo.get('__search_key__'), {'expected_cost': wo_expected, 'actual_cost': wo_actual})
            ...
            server.update(proj.get('__search_key__'), {'expected_cost': proj_expected, 'actual_cost': proj_actual})
        ...
        server.update(title.get('__search_key__'), {'expected_cost': title_expected, 'actual_cost': title_actual})
    #Update the order costs here            
    order_actual = float(order_dict['actual_cost'])
    order_expected = float(order_dict['expected_cost'])
    server.update(order.get('__search_key__'), {'expected_cost': order_expected, 'actual_cost': order_actual})
    #print "ORDER CODE = %s" % order_code
    #print "ORDER DICT = %s" % order_dict
    os.system('echo "%s" >> done_orders' % order_code)
    time.sleep(1)
