...
def main(server=None, input=None):
    ...
    try:
        ...
        # This is to keep a record on the source of which work orders have used it as a deliverable
        # input is a work_order_deliverables entry
        sobj = input.get('sobject')
        sk = input.get('search_key')
        source_code = sobj.get('deliverable_source_code')
        ...
        deliverable_for = source.get('deliverable_for')
        new_str = deliverable_for
        if deliverable_for.find(work_order_code) == -1:
            new_str = '%s,%s' % (new_str, work_order_code)
        if deliverable_for.find(title_code) == -1:
            new_str = '%s,%s' % (new_str, title_code)
        if deliverable_for.find(order_code) == -1:
            new_str = '%s,%s' % (new_str, order_code)
        if new_str[0] == ',':
            new_str = new_str[1:]
        server.update(source.get('__search_key__'), {'deliverable_for': new_str})
