...
def main(server=None, input=None):
    ...
    try:
        ...
        new_pipe = update_data.get('pipeline_code')
        old_pipe = prev_data.get('pipeline_code')
        ...
        saved_task_info = {}
        found = []
        if new_pipe != old_pipe and st in ['twog/title','twog/proj']:
            if st == 'twog/proj':
            ...
            spt_processes_expr = "@SOBJECT(config/process['pipeline_code','%s'])" % (new_pipe)
            spt_processes = server.eval(spt_processes_expr)
            kids_expr = ''
            kids = []
            if st == 'twog/title':
            ...
            if pipe_create_method != 'append':
                ...
                for p in spt_processes:
                    pr = p.get('process')
                    for kid in kids:
                        if p.get('process') == kid.get('process'):
                ...
                for kid in kids:                                          
                    if kid.get('process') not in future_retires.keys():   
            ...
            for p in spt_processes:
                ...
                if p.get('process') in saved_task_info.keys():
                    mr_t_data = { 
                                  ...
                                  'discussion': saved_task_info[pr].get('discussion'),
            ...
            if st == 'twog/title':
                if pipe_create_method != 'append':
                    ...
                    title_prereq_expr = "@SOBJECT(twog/title_prereq['title_code','%s']['pipeline_code','%s'])" % (sob_code,old_pipe)
                ...
                if new_pipe not in ['',None]:
                    if new_pipe.find('NOTHINGXsXNOTHING') == -1:
                        ...
                        #Now do all Work Orders. Give them the same prereqs
