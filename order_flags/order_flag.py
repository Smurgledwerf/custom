__all__ = ["OrderFlagWdg"]
import tacticenv
from pyasm.common import Environment
from pyasm.web import Table, DivWdg
from tactic.ui.common import BaseTableElementWdg

class OrderFlagWdg(BaseTableElementWdg):

    def init(my):
        nothing = 'true'
        my.unseen_txt = '<font color="#FF0000"><b>NEW!</b></font>';
        my.seen_txt = '<font color="#000000"><b>Mark Unseen</b></font>';
    ...
    def get_indicate_behavior(my, login, code):
        behavior = {'css_class': 'clickme', 'type': 'click_up', 'cbjs_action': '''        
                        try{
                          ...
                          var seen_txt = '<font color="#000000"><b>Mark Unseen</b></font>';
                          var server = TacticServerStub.get();
                          var order = server.eval("@SOBJECT(twog/order['code','" + code + "'])")[0];
                          var seen_by = order.seen_by;
                          if(seen == '0'){
                              ...
                              server.update(order.__search_key__, {'seen_by': new_seen_by}); 
                             ...
                             var prev_seen = order.prev_seen_by + move_str;
                             server.update(order.__search_key__, {'prev_seen_by': prev_seen, 'seen_by': less_str});                             
