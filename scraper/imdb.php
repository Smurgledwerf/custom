...
{	
	...
	public function getMultipleMovieInfo($title, $getExtraInfo = true)
	{
		$imdbIds = $this->getMultipleIMDbIdFromSearch(trim($title));
		if(is_array($imdbIds)){
			if(count($imdbIds) == 0){
				$imdbIds = NULL;
			}
		}
		if($imdbIds == NULL || $imdbIds == ''){
		    $arr = array();
		    $arr['error'] = "No Title found in Search Results!";
		    return $arr;
		} 
		...
		foreach($imdbIds as $key=>$imdbId){
                        $new_info = $this->getMovieInfoById($imdbId, $getExtraInfo);
			array_push($arr, $new_info);
		}
		return $arr;
	}
	...
	// Movie title search on Google, Bing or Ask. If search fails, return FALSE.
	private function getMultipleIMDbIdFromSearch($title, $engine = "google"){
		switch ($engine) {
			case "google":  $nextEngine = "bing";  break;
			case "bing":    $nextEngine = "ask";   break;
			case "ask":     $nextEngine = FALSE;   break;
		...
		$ids = $this->match_all('/<a.*?href="http:\/\/www.imdb.com\/title\/(tt\d+).*?".*?>.*?<\/a>/ms', $this->geturl($url), 1);
		if (!isset($ids[0]) || empty($ids[0])) //if search failed
			return $this->getMultipleIMDbIdFromSearch($title, $nextEngine); //move to next search engine
		else
			return $ids; //return first IMDb result
	}
